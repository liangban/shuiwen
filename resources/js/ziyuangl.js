layui.use(['api', 'jquery', 'form', 'layer', 'laydate'], function() {
	...
	form.on('submit(formDemo)', function(data) {
		...
		api.resourceData(resourceID, stationCode, function(datas) {
			if(datas.status == true) {
				var html = ''
				$.each(datas.data, function(i, v) {
					html += '<ul class="row">' +
						'                <li class="col-1">' + v.resourceDataID + '</li>' +
						'                <li class="col-3">' + v.resourceDataName + '</li>' +
						'                <li class="col-1">' + v.startDate + '</li>' +
						'                <li class="col-1">' + v.endDate + '</li>' +
						'                <li class="col-1">' + v.stationName + '</li>' +
						'                <li class="col-1">' + v.resourceName + '</li>' +
						'                <li class="col-2"><button class="remove" data-resourceDataID='+ v.resourceDataID + '>删除</button></li>' +
						'            </ul>';
				})
	...
	form.on('submit(formDemo1)', function(data) {
		...
		var endDate = $('#end').val();
		api.detil(resourceID, stationCode, startDate, endDate, function(data) {
			if(data.status == true) {
				layer.msg(data.msg)
	...
	$(document).on('click','.data_content .remove',function(){
		var resourcedataid=$(this).attr('data-resourcedataid');
		var that=$(this)
		api.deleteData(resourcedataid,function(data){
			if(data.status==true){
